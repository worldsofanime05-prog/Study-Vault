...
(function () {
    ...
    function injectStyles() {
        ...
        #sv-ai-panel {
            ...
            box-shadow: -8px -4px 40px rgba(0,0,0,0.5);
            z-index: 1201;
            display: flex;
        ...
        .sv-ai-resize-handle:hover::after,
        .sv-ai-resize-handle:active::after {
        ...
        [data-theme="light"] #sv-ai-panel {
            border-left-color: rgba(117,91,0,0.15);
            border-top-color: rgba(117,91,0,0.15);
        }
        ...
        #sv-global-ai-fab.sv-fab-dragging {
            cursor: grabbing;
            transform: scale(1.1);
    ...
    function createPanel() {
        ...
        panel.innerHTML = `
        <div class="sv-ai-resize-handle" id="sv-ai-resize-handle"></div>
        <div id="sv-ai-header">
        ...
        `;
        // Note: click is handled by makeFabDraggable (tap vs drag detection) 

        document.body.appendChild(backdrop);
        document.body.appendChild(panel);
        document.body.appendChild(fab);

        // ── DRAGGABLE FAB — position can be changed by dragging ──
        makeFabDraggable(fab);

        // ── RESIZABLE PANEL — drag top handle to resize ──
    ...
    // ── DRAGGABLE FAB LOGIC ──────────────────────────────────
    // Makes the AI FAB button draggable so the user can reposition it anywhere on screen.
    // Distinguishes between a tap (opens panel) and a drag (repositions).
    function makeFabDraggable(fab) {
        let isDragging = false;
        let hasMoved = false;
        let startX = 0, startY = 0;
        let fabStartX = 0, fabStartY = 0;
        const DRAG_THRESHOLD = 5; // px — movement beyond this = drag, not tap

        // Restore saved position
        try {
            const saved = localStorage.getItem('sv_fab_pos');
            if (saved) {
        ...
        function onStart(e) {
            ...
            isDragging = true;
            hasMoved = false;
            fab.classList.add('sv-fab-dragging');
            e.preventDefault();
        }
        ...
        function onMove(e) {
            ...
            if (hasMoved) {
                ...
                newX = Math.max(0, Math.min(window.innerWidth - fabW, newX));
                newY = Math.max(0, Math.min(window.innerHeight - fabH, newY));
        ...
        function onEnd(e) {
            if (!isDragging) return;
            isDragging = false;
            fab.classList.remove('sv-fab-dragging');
            if (hasMoved) {
                // Save position
                try {
                    localStorage.setItem('sv_fab_pos', JSON.stringify({
        ...
        // Prevent any stray click events from firing during/after drag
        ...
        // Ensure position is valid after window resize
        window.addEventListener('resize', function() {
            const rect = fab.getBoundingClientRect();
            if (rect.left > window.innerWidth - fab.offsetWidth) {
                fab.style.left = (window.innerWidth - fab.offsetWidth) + 'px';
            }
            if (rect.top > window.innerHeight - fab.offsetHeight) {
                fab.style.top = (window.innerHeight - fab.offsetHeight) + 'px';
            }
        });
    }

    // ── RESIZABLE PANEL LOGIC ────────────────────────────────
    // Allows the user to drag the top handle to resize the chat panel height.
    function makePanelResizable(panel) {
        const handle = document.getElementById('sv-ai-resize-handle');
        if (!handle) return;
        ...
        let startH = 0;
        const MIN_H = 200;
        const MAX_H_RATIO = 0.9; // 90vh

        // Restore saved height
        try {
            const saved = localStorage.getItem('sv_panel_height');
            if (saved) {
                const h = parseInt(saved);
                if (h >= MIN_H && h <= window.innerHeight * MAX_H_RATIO) {
        ...
        function onResizeStart(e) {
            ...
            panel.style.transition = 'none'; // disable smooth transition during resize
        ...
        function onResizeEnd() {
            ...
            // Save height
            try {
                localStorage.setItem('sv_panel_height', parseInt(panel.style.height));
            } catch(_) {}
        }
